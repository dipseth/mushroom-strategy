 ...
 *
 * Modifications: hardcoded element name to `mushroom-strategy-area-card` to
 * avoid clashing with the HACS-installed `minimalistic-area-card`, and removed
 * the package.json version import so this can be bundled inside the strategy.
 */
...
/* eslint no-console: 0 */
console.info(
    `%c  Mushroom Strategy Area Card  %c (forked from minimalistic-area-card) `,
    ...
    formatNumber(
        ...
        Number.isNaN =
            ...
            function isNaN(input: unknown): boolean {
                return typeof input === "number" && isNaN(input as never);
            };
